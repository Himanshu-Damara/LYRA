  ...
  try {
    ...
    if (resp.ok) {
      ...
      if (ollama?.models?.length) {
        ...
        if (!ollama.models.includes(runModel.value)) runModel.value = ollama.models[0]
      }
    }
  } catch {}
}
...
onMounted(async () => {
  const devicesPromise = fetch('/api/phone/devices')
    .then(resp => resp.ok ? resp.json() : null)
    .then(data => {
      const devices = Array.isArray(data?.devices) ? data.devices : []
      if (devices.length && !devices.some((d: any) => d.serial === runDevice.value)) {
        runDevice.value = devices[0].serial
      }
    })
    .catch(() => {})
  await Promise.all([store.fetchSuites(), store.fetchTasks(), store.fetchRuns(), devicesPromise])
        ...
        <div v-show="activeSubTab === 'tasks'" class="space-y-4">
          ...
          <div class="card" style="min-height: 200px">
            <div class="flex items-center justify-between mb-3">
              <h2 class="text-base font-semibold" style="color: var(--text-1)">LYRA Bench</h2>
              <span class="text-xs" style="color: var(--text-4)">{{ store.tasks.length }} tasks Â· {{ runPlatform }}</span>
            </div>
            ...
            <div v-if="run.results?.length">
              <div v-for="r in run.results" :key="r.task_id" class="border-t" style="border-color: var(--border)">
                <div class="flex items-center gap-3 py-2 cursor-pointer hover:bg-white/[0.02] px-1 rounded"
                  ...
                  <span class="text-xs" style="color: var(--text-4)">{{ expandedLog === r.task_id ? 'â–¼' : 'â–¶' }}</span>
                ...
                <div v-if="expandedLog === r.task_id && r.agent_log?.length" class="mb-3 ml-4 p-3 rounded-lg"
                  ...
                  <div style="display: flex; flex-direction: column; gap: 4px">
                    <template v-for="(ev, j) in r.agent_log" :key="j">
                      ...
                      <div v-else-if="ev.type === 'tool_call'" style="font-size: 10px; color: #38bdf8; padding: 2px 8px; background: #0ea5e908; border-radius: 10px; border: 1px solid #0ea5e922; width: fit-content">ðŸ”§ {{ ev.name }}({{ JSON.stringify(ev.args || {}).slice(0, 100) }})</div>
                      <div v-else-if="ev.type === 'tool_result'" style="font-size: 9px; font-family: monospace; color: #64748b; padding: 3px 8px; max-height: 80px; overflow: hidden; border-left: 2px solid #1e293b; margin-left: 8px; white-space: pre-wrap; word-break: break-all">â†³ {{ ev.result?.slice(0, 300) || ev.name }}</div>
        ...
        <div class="card p-2" style="flex: 1; min-height: 150px; overflow: hidden; display: flex; flex-direction: column">
          ...
          <div style="flex: 1; overflow-y: auto; display: flex; flex-direction: column; gap: 3px; padding-right: 4px">
            ...
            <template v-for="(ev, i) in liveLog" :key="i">
              ...
              <div v-else-if="ev.type === 'agent' && ev.name && !ev.result" style="font-size: 9px; color: #38bdf8; padding: 2px 6px; background: #0ea5e908; border-radius: 8px; border: 1px solid #0ea5e922; width: fit-content">ðŸ”§ {{ ev.name }}</div>
              <div v-else-if="ev.type === 'agent' && ev.result" style="font-size: 8px; font-family: monospace; color: #475569; padding: 1px 6px; border-left: 2px solid #1e293b; margin-left: 6px; max-height: 40px; overflow: hidden; word-break: break-all">â†³ {{ ev.result?.slice(0, 150) }}</div>
              <div v-else-if="ev.type === 'task_result'" style="font-size: 10px; font-weight: 600; padding: 3px 6px; border-radius: 6px"
                ...
                {{ ev.score > 0 ? 'PASS' : 'FAIL' }} â€” {{ ev.reason }}
              </div>
