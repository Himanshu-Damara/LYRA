...
<template>
  <div class="task-review">
    <div class="review-header">
      <div><div class="eyebrow">QUALITY GATE · GENERATED CONTENT</div><h2>Agent Task Review</h2><p>Complete sources, review generated sections, and export a persistent review packet.</p></div>
      <div class="header-actions"><span v-if="savedAt" class="saved-state">✓ Saved {{ savedAt }}</span><button class="primary-btn" :disabled="!selectedTask" @click="downloadPacket">↓ Download review packet</button></div>
    </div>
    <div v-if="error" class="error-box">{{ error }}</div>
    <div class="filter-bar">
      <label class="search-field">⌕ <input v-model="query" placeholder="Search task, section, source, or generated content…" /></label>
    ...
    <div class="review-stats"><div><span>{{ tasks.length }}</span><small>Matching tasks</small></div><div><span>{{ tasks.filter(t => t.ready).length }}</span><small>Ready</small></div><div><span>{{ tasks.filter(t => !t.ready).length }}</span><small>Missing data</small></div><div><span>{{ loading ? '…' : (selectedTaskIndex >= 0 ? selectedTaskIndex + 1 : 0) }}</span><small>Selected</small></div></div>

    <div class="review-layout">
      <aside class="task-list card"><div class="list-heading"><strong>Task records</strong><span>{{ tasks.length }} results</span></div><div v-if="loading" class="empty-state">Loading task records…</div><template v-else><button v-for="task in tasks" :key="task.id" class="task-row" :class="{ selected: task.id === selectedTask?.id }" @click="selectTask(task.id)"><div class="task-row-top"><strong>{{ task.title }}</strong><span class="status-pill" :class="task.status.toLowerCase().replace(' ', '-')">{{ task.status }}</span></div><p>{{ task.summary }}</p><div class="task-meta"><span>{{ task.owner }} · {{ task.agent }}</span><span>{{ task.completion.complete }}/{{ task.completion.total }}</span></div><div class="progress"><i :style="{ width: `${task.completion.percent}%` }"></i></div><span v-if="!task.ready" class="missing-label">! Missing required data</span></button><div v-if="!tasks.length" class="empty-state">No agent tasks match the current filters.<br /><button class="reset-btn" @click="resetFilters">Reset filters</button></div></template></aside>
