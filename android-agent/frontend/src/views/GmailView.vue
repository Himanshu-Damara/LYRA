...
<template>
  <div class="gmail-page">
    <div class="gmail-header"><div><div class="eyebrow">GOOGLE WORKSPACE INTEGRATION</div><h2>Gmail</h2><p>Use Grok with Gmail after Google OAuth authorization.</p></div><div v-if="connected" class="connected-pill">● {{ email }}</div></div>
    ...
    <section v-if="connected" class="card assistant-card"><div class="section-title"><div><strong>Ask Gmail AI</strong><span>Search/read relevant mail and prepare a draft; sending always requires your confirmation.</span></div></div><textarea v-model="assistantPrompt" rows="3" placeholder="Example: Find the latest mail from Alex and draft a polite reply…"></textarea><button class="primary" :disabled="assistantBusy || !assistantPrompt.trim()" @click="askAssistant">{{ assistantBusy ? 'Thinking…' : 'Ask AI' }}</button><p v-if="assistantReply" class="assistant-reply">{{ assistantReply }}</p></section>
    <section v-if="connected" class="card compose-card"><div class="section-title"><div><strong>Compose email</strong><span>Direct Gmail API send · Android UI is not used</span></div></div><label>To<input v-model="to" type="email" placeholder="recipient@example.com" /></label><label>Subject<input v-model="subject" placeholder="Subject" /></label><label>Message<textarea v-model="body" rows="9" placeholder="Write your message…"></textarea></label><button class="primary send" :disabled="busy || !to || !subject || !body" @click="send">{{ busy ? 'Sending…' : 'Send email' }}</button></section>
  </div>
</template>
