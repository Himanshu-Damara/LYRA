...
class Device:
    ...
    def dump_xml(self) -> str:
        ...
        # UIAutomator can hang when the device is busy or an accessibility
        # provider is wedged. Keep screen reads bounded so agent requests do
        # not remain stuck at “Reading screen...” forever.
        self.adb("shell", "uiautomator", "dump", "/sdcard/tt.xml", timeout=8)
        r = subprocess.run(
    ...
    def restart_tiktok(self, activity=TIKTOK_MAIN_ACTIVITY):
        ...
        if (
            ...
            or "Activity not started" in r.stdout
            or self.screen_type(self.dump_xml()) == "unknown"
        ):
            self.adb("shell", "monkey", "-p", TIKTOK_PKG, "-c", "android.intent.category.LAUNCHER", "1", timeout=10)
        time.sleep(3)
