...
def _packet(task, sections) -> str:
    ...
    for s in data["sections"]:
        ...
        lines.append(f"- [{mark}] {s['name']} ({'required' if s['required'] else 'optional'}) â€” {s['status']}; source: {s['source'] or 'MISSING'}")
...
def seed_review_tasks() -> None:
    ...
    seeds = [
        ...
        ("task-settings-003", "Enable dark mode in Settings", "Navigate to Android display settings and enable dark theme.", "Mobile QA", "Settings Explorer", "Draft", "OS version is not recorded.", ["No postcondition captured."], [
            ...
            ("steps", "Generated steps", "Settings Explorer", "review", "Open Settings â†’ Display â†’ Dark theme.", "Confirm label by Android version."), ("validation", "Validation & review", "", "missing", "", "No postcondition captured."),
        ]),
    ]
    db = SessionLocal()
    try:
