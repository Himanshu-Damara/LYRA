"""Agent Chat routes â€” interactive natural language phone control."""
...
@router.post("/stop-device/{device}", summary="Stop Any Agent on Device")
def stop_device_agent(device: str):
    """Stop whichever agent task currently owns this phone."""
    from gitd.services.agent_chat import stop_device_turn

    session_id = stop_device_turn(device)
...
def send_message(data: dict = Body({})):
    ...
    # Session not in memory â€” try to reload from DB (happens after backend restart)
    ...
    if not claim_device_turn(session.device, session.id):
        raise HTTPException(
            status_code=409,
            detail="This device already has an agent task running. Wait for it to finish or press Stop first.",
        )
    ...
    def generate():
        ...
        try:
            ...
            # ALWAYS clean up â€” whether stream completed normally or was aborted
            try:
            ...
            try:
                ...
                pass
            release_device_turn(session.device, session.id)
            clear_stop_request(session.id)
...
# â”€â”€ Conversation persistence endpoints â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
...
def ollama_pull(req: OllamaModelRequest):
    """Pull (download) a model from the Ollama registry. Non-blocking â€” runs in background."""
